...
if (!$loggedin) { // Check if the user is not logged in
            ...
            if ($res) {
                if ($res->num_rows > 0) {
                    ...
                    if ($adminPosition === 'Manager') {
                        echo '<button class="btn-create-new" onclick="createNew() "><i class="fas fa-plus"></i></button>';
                    }
                        echo '</div>
                    </div>
                    <br>';
                                ...
                                while ($row = $res->fetch_assoc()) {
                                    echo '<tr>
                                        ...
                                        if ($positionResult && $positionRow = $positionResult->fetch_assoc()) {
                                            ...
                                            if ($adminPosition === 'Manager') {
                                                echo ' <button onclick="editUser('. $row['UserID'] .')" class="btn-track-info"><i class="fas fa-pen"></i></button>
                                                <a href="User-Delete.php?id='. $row['UserID'] .'" onclick="return confirm(\'Are you sure to delete?\');" class="btn-track-info"><i class="fas fa-trash"></i></a>';
                                            } else {
    ...
    <!-- Pop up window to create new user -->
    <div class="newUser" id="newUser">
            <form id="userForm" class="userForm" action="User-Create.php" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
                ...
                <div class="acc-cont">
                    ...
                    <div class="acc-detail2">
                        <p><input type="text" name="name" class="acc-input" placeholder="Username" required></p>
                        <p><textarea name="userbio" class="acc-bio-input" rows="3" cols="50" maxlength="100" placeholder="User Biography. Within 100 characters." required></textarea></p>
                        <p><input type="email" name="email" class="acc-input" placeholder="User Email" required></p>
                        <p><input type="password" id="enter-ps" name="password" class="acc-input" placeholder="User Password" required pattern="^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*()_+{}\[\]:;<>,.?~\\/-]).{8}$"
                        ...
                        <span id="pwd-requirement"></span><br>
                        <input type="submit" class="button-acc" name="createUser" value="Create">
                    </div>
                </div>                      
            </form>
        </div>';

        echo ' <!-- Pop up window to edit user -->
        <div class="editUser" id="editUser">
            <form id="userForm" class="userForm" action="User-Update.php" method="POST" enctype="multipart/form-data">
                <span id="closeButton" onclick="hidePopup()"><i class="fa fa-close"></i></span>
                <div class="acc-cont">
                    <div class="acc-detail1">
                        <div class="image-upload-wrap">
                    ...
                    <div class="acc-detail2">
                        <p><input type="text" name="name" id="name" class="acc-input" placeholder="Username" required></p>
                        <p><textarea name="userbio" id="bio" class="acc-bio-input" rows="3" cols="50" maxlength="100" placeholder="User Biography. Within 100 words." required></textarea></p>
                        <p><input type="email" name="email" id="email" class="acc-input" placeholder="User Email" required></p>
                        <p><input type="password" name="password" id="password" class="acc-input" placeholder="User Password" disabled></p>
                        <br>
                        <input type="hidden" name="userid" id="userid">
                        <input type="submit" class="button-acc" name="updateUser" value="Update">
                    </div>
                </div>                      
            </form>
        </div>';
       ...
       function createNew() {
        ...
        function editUser(id) {
            ...
            success: function(response) {
                ...
                var imagePath = "Data/User/" + response.img;
