...
<?php
  ...
  if(isset($_POST["email"]) && (!empty($_POST["email"]))){
    ...
    if (!$email) {
      ...
      $sel_query = "SELECT * FROM user WHERE UserEmail='" . $email . "'";
    ...
    if ($error != "") {
      ...
      $res = mysqli_query($con, "SELECT * FROM user WHERE UserEmail='" . $email . "'");
      if (!$res) {
        die("Error in querying the user table: " . mysqli_error($con));
      }

      // Insert into reset_password_user table
       $stmt = $con->prepare("INSERT INTO password_reset_user (UserID, Email, `Key`, ExpDate) VALUES (?, ?, ?, ?)");
      if (!$stmt) {
      ...
      if ($res->num_rows > 0) {
        ...
        $stmt->bind_param("isss", $userID, $email, $tokenReset, $expiration);
        if (!$stmt->execute()) {
      ...
      $output.='<p><a href="http://localhost/WebMusicPlayer/Password-Reset.php?tokenReset='.$tokenReset.'&email='.$email.'&action=reset" target="_blank">http://localhost/WebMusicPlayer/Password-Reset.php?tokenReset='.$tokenReset.'&email='.$email.'&action=reset</a></p>';		
      ...
      $output.='<p>MuziQ</p>';
      $body = $output; 
      $subject = "MuziQ - Password Reset Request";
