...
<?php
    ...
    if (isset($_GET["tokenReset"]) && isset($_GET["email"])
        ...
        $query = mysqli_query($con, "SELECT * FROM password_reset_user WHERE `Key`='$tokenReset' and `Email`='$email';");
        ...
        if ($row == 0) {
            $error .= '<h2>Invalid Link</h2>
                ...
                <p><a href="Password-Forgot.php">Click here</a> to reset password.</p>';
        } else {
...
<?php
    ...
    if (isset($_POST["email"]) && isset($_POST["action"]) && ($_POST["action"] == "update")) {
        ...
        if ($error != "") {
            ...
            // Update the user password and delete the reset entry
            mysqli_query($con,
                "UPDATE user SET UserPassword='$pwd2', trn_date='$curDate' 
                WHERE UserEmail='$email';"
            );

            mysqli_query($con, "DELETE FROM password_reset_user WHERE Email='$email';");
